  ...
  return (
    <section id='experience' className='container min-h-screen w-full px-8 pt-[68px] flex items-center justify-center'>
      <div className='text-center flex flex-col items-center'>
        ...
        <div className='mt-8'>
          <VerticalTimeline lineColor={theme === 'light' ? '#94a3b8' : '#cbd5e1'} className='before:!left-2 xs:before:!left-5 screen1174:before:!left-[50%]'>
    ...
    <VerticalTimelineElement
      ...
      icon={
        <div className='flex justify-center items-center w-full h-full'>
          <img src={experience.icon} alt={experience.companyName} className='w-[60%] h-[60%] object-contain opacity-80' />
        </div>
      }
    >
      ...
      <ul className='mt-5 list-disc ml-5 space-y-2 text-left'>
        {experience.points.map((point, index) => (
          <li key={`experience-point-${index}`} className='text-secondaryText dark:text-dark-secondaryText text-[14px] pl-1 tracking-wider'>
            {point}
          </li>
        ))}
      </ul>
